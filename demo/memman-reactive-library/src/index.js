...
export default class ReactiveComponent extends ReactiveElement {
    _parent = null;
...
export function createElement(type, props, ...children) {
    ...
    if (typeof type === "string") {
        ...
        Object.entries(props).forEach(([key, value]) => {
          if (key === "className") {
            ...
            if (typeof value !== "object") {
              throw new Error(`El valor de la propiedad 'style' debe ser un objeto`);
            }
            Object.entries(value).forEach(([styleKey, styleValue]) => {
              if (typeof styleValue !== "string") {
                throw new Error(`El valor de la propiedad 'style.${styleKey}' debe ser una cadena`);
              }
      ...
      element = new type(props, ...children);
      if (element.render) {
    ...
    const childNodes = children
      ...
      .map((child) => {
        if (child instanceof ReactiveElement) {
          ...
          throw new Error(`Unsupported child node: ${child}`);
        }
      });
...
function render(component, container) {
    ...
    if (typeof component.render !== 'function') {
        // Si component no tiene un método render, asume que component ya es un elemento del DOM
        if (container) {
